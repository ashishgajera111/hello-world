...
class Amasty_Table_Model_Rate extends Mage_Core_Model_Abstract
{
    ...
    protected $_bigpostShippingTotal = 0;
	protected $_kg_shipping_type = 7; // Here need to put shipping type id which want to call bigpost api
    ...
    public function findBy($request, $collection)
    {
        ...
        foreach ($items as $item) {
			...
			//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
			if ($product->getAmShippingType() == $this->_kg_shipping_type) 
			{
				$shippingaddress=array(
					'ToSuburb'=>$request->getDestCity(),
					"ToPostcode" => $request->getDestPostcode(),
					"Tostate" => $request->getDestRegionCode(),
				);
				
				$product = Mage::getModel('catalog/product')->load($item->getProduct()->getId());
				$deliveryItem[]=array(
					'itemDescription' => $product->getDescription(),
					'deadWeight' => $product->getWeight(),
					'depth'=>10,
					'height' => 10,
					'quantity' => $item->getQty(),
					'width' => 30);
		
				$shippingPrice = $this->_getshippingrate($shippingaddress,$deliveryItem);
				$this->_bigpostShippingTotal += $shippingPrice;
			}
			///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
    ...
    protected function calculateCosts($allRates, $totals, $request, $shippingType)
    {
        ...
        foreach ($allRates as $rate) {
			...
			if($shippingType == $this->_kg_shipping_type)
			{
				$results[$id]['cost'] = $this->_bigpostShippingTotal;
                $results[$id]['time'] = $rate['time_delivery'];	
			}
	...
	public function _getshippingrate($shippingaddress,$deliveryItem)
	{
		//echo "<pre>";print_r($shippingaddress);die;
	   $url = "https://www.bigpost.com.au/api/requestquote";
	   $headers = array(
	   'Content-Type: application/json',
	   'AccessToken: EF7FB30623BE468B8253AC5B93ACA8F4'
	   );

	   

	   $data = array("DeliveryType" => "1",
	   "DepotId" =>"876",
	   "FromPostcode" => "3121",
	   "FromSuburb" => "Richmond",
	   "FromState" => "VIC",
	   "ToSuburb" => $shippingaddress['ToSuburb'],
	   "ToPostcode" => $shippingaddress['ToPostcode'],
	   "Tostate" => $shippingaddress['Tostate'],
	   "DoNotPriceAtlJobs" => "false",
	   "DeliveryItems" => $deliveryItem);
		...
		return str_replace('$','',$result_new['Total']);	
		//echo '<pre>';print_r($result_new['Total']);
		//die;
	}
