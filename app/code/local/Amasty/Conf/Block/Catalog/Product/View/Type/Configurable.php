...
class Amasty_Conf_Block_Catalog_Product_View_Type_Configurable extends Mage_Catalog_Block_Product_View_Type_Configurable
{
    ...
    private $priceElements = array('product-price-', 'price-excluding-tax-', 'price-including-tax-');
    const SMALL_SIZE = 50;
    const BIG_SIZE = 100;
    ...
    protected function getSettingConfig()
    {
        ...
        $confData = array(
            ...
            'isShowAllOptions'   => $this->getModuleConfig('general/show_all')
        );
    ...
    protected function _generateConfig()
    {
        ...
        $defaultPriceProduct = $this->_getPriceBlock($this->getProduct()->getTypeId())->getProduct();
        $confData['default_price_incl_tax'] = Mage::helper('core')->currency(
            Mage::helper('tax')->getPrice($defaultPriceProduct, $defaultPriceProduct->getFinalPrice(), true),
            false,
            false
        );
        $confData['default_price_excl_tax'] = Mage::helper('core')->currency(
            Mage::helper('tax')->getPrice($defaultPriceProduct, $defaultPriceProduct->getFinalPrice(), false),
            false,
            false
        );
    ...
    protected function generateChildConfig(Mage_Catalog_Model_Product $simple, $reloadValues)
    {
        ...
        if ($this->_useSimplePrice) {
            ...
            $priceHtml = $this->generatePriceHtml($simple);
            $priceCloneHtml = $this->generatePriceHtml($simple, true);
            foreach ($this->priceElements as $priceElement) {
                $priceHtml = $this->replacePriceHtml($simpleId, $priceElement, $priceHtml);
                $priceCloneHtml = $this->replacePriceHtml($simpleId, $priceElement, $priceCloneHtml);
            }

            $result['price_html'] = $priceHtml;
            $result['price_clone_html'] = $priceCloneHtml;
    ...
    public function getJsonConfig()
    {
        ...
        $swCollection = $this->getAmastyHelper()->getSwatchData();
                    ...
                    if (in_array($attributeId, $productImagesAttributes)) {
                        foreach ($option['products'] as $productId) {
                            ...
                            if ($image && $image !== "no_selection") {
                                $config['attributes'][$attributeId]['options'][$i]['bigimage'] = (string)Mage::helper('catalog/image')
                                    ->init($productParent, 'image', $image)
                                    ->keepAspectRatio(true)
                                    ->keepFrame(true)
                                    ->watermark(null, null)
                                    ->resize($bigWidth, $bigHeight);

                                $config['attributes'][$attributeId]['options'][$i]['image'] = (string)Mage::helper('catalog/image')
                                    ->init($productParent, 'image', $image)
                                    ->keepAspectRatio(true)
                                    ->keepFrame(true)
                                    ->watermark(null, null)
                        ...
                        $swatchModel = isset($swCollection[$option['id']]) ? $swCollection[$option['id']] : null; 
                        $imgUrl = $this->getAmastyHelper()
                            ->getImageUrl($option['id'], $smWidth, $smHeight, $swatchModel);
                        $tooltipUrl = $this->getAmastyHelper()
                            ->getImageUrl($option['id'], $bigWidth, $bigHeight, $swatchModel);
                        if ($imgUrl == "") {
    ...
    /**
     * Replace simple id to parent id in price html
     *
     * @param $simpleId
     * @param $priceElement
     * @param $priceHtml
     * @return string
     */
    private function replacePriceHtml($simpleId, $priceElement, $priceHtml)
    {
        $parentId = $this->getProduct()->getId();

        $priceHtml = str_replace($priceElement . $simpleId, $priceElement . $parentId, $priceHtml);

        return $priceHtml;
    }

    /**
     * Generate price html of simple product
     *
     * @param $simple
     * @param bool $clone
     * @return string
     */
    private function generatePriceHtml($simple, $clone = false)
    {
        if ($clone) {
            $priceHtml = $this->getPriceHtml($simple, false, '_clone');
        } else {
            $priceHtml = $this->getPriceHtml($simple);
        }
        $priceHtml .= $this->getTierPriceHtml($simple);
