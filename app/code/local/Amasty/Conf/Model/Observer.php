...
class Amasty_Conf_Model_Observer 
{
    public function onListBlockHtmlBefore($observer)//core_block_abstract_to_html_after    
    {
        if (!($observer->getBlock() instanceof Mage_Catalog_Block_Product_List
            ...
            && strpos($observer->getBlock()->getTemplate(), "list.phtml") !== FALSE
            )) {
            return;
        }
        ...
        if(strpos($html, "amconf-block") !== FALSE)
            return;
    ...
    public function onSuperProductAttributesConfigurationAfter($observer) {//core_block_abstract_to_html_after   
        if (($observer->getBlock() instanceof Mage_Adminhtml_Block_Catalog_Product_Edit_Tab_Super_Config)){
            $html = $observer->getTransport()->getHtml();
            if(strpos($html, "use_image_from_product") !== FALSE) {
                return;
            }
    ...
    public function onSuperProductAttributesPrepareSave($observer){
        $data = $observer->getRequest()->getParam('configurable_attributes_data');
        if (!$data) {
            return;
        }
        $configurable_attributes_data = Mage::helper('core')->jsonDecode($data);
    ...
    public function addHandle(Varien_Event_Observer $observer)
    {
        try {
            ...
            if (is_array($handles)
                && (in_array('catalog_product_view', $handles)
                    || in_array('review_product_list', $handles)
                    || in_array('amquickview_ajax_view', $handles)
                )
            ) {
                ...
                if ($isModuleEnable) {
                    $observer->getEvent()->getLayout()->getUpdate()
                        ->addHandle('amconf_zoom_lightbox_handle');
                }
            }
        } catch (Exception $ex) {
            return;
