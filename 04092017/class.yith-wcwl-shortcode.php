...
if( ! class_exists( 'YITH_WCWL_Shortcode' ) ) {
	...
	class YITH_WCWL_Shortcode {
		...
		session_start();
			...
			$uid=$_SESSION['guser_id'];
			 //echo $_SESSION['guser_id'];
...
//print_r($atts);
			...
			if(
				...
				*/
				...
				//echo "<pre>";print_r($wishlist_id)exit;
...
//echo "<pre>";print_r($yith_wcwl_wishlist_token);exit;
global $wpdb;
$sql="select * from ".$wpdb->prefix."yith_wcwl_lists where wishlist_token='".$yith_wcwl_wishlist_token."'";
$meuser_id=$wpdb->get_results($sql);
$user_id=$meuser_id[0]->user_id;
//$user_id=$meuser_id;
				$is_user_owner = false;
				$query_args = array();
...
$wishlist_meta = YITH_WCWL()->get_wishlist_detail_by_token( $wishlist_id );
				...
				//echo "<pre>";print_r($wishlist_meta);exit;
...
//echo "<pre>";print_r($action);exit;
		...
		public static function add_to_wishlist( $atts, $content = null ) {
			...
			// default wishlist id
			$default_wishlists = YITH_WCWL()->get_wishlists( array( 'is_default' => true ) );
